...
for (rcp in c("rcp26", "rcp45", "rcp85")){
  ...
  for (rcm in as.character(c(1:6))){
    for(p in c("H", "N", "E")){
      n_dir <- paste0(tmp_result_path, "/", rcp, "_", "rcm", rcm, "_", p)
      dir.create(n_dir, recursive = TRUE)
      files_to_copy <- list.files(paste(cli_dir, rcp, rcm, p, sep = "/"), full.names = TRUE)
      file.copy(files_to_copy, n_dir, overwrite = TRUE)
      print(paste("Copied climate files for", rcp, rcm, p))
    }
  }
}
